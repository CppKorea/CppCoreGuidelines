...
swap함수을 이용해서 복사 대입을 구현하는 것을 고려하라. [소멸자, 자원 해제, 그리고 swap은 실패해선 안된다](./Errors.md#Re-never-fail).
...
### <a name="Rc-memset"></a>C.90: `memset` 또는 `memcpy`가 아닌, 생성자와 할당 연산자에 의존하라

##### Reason

타입의 개체를 생성하기 위한 표준 C++ 메커니즘은 생성자를 호출하는 것이다.

가이드라인에 명시된 것 처럼 [C.41](#Rc-complete) : 생성자는 완전히 초기화된 개체를 생성해야 한다.  
...
이로 인해 복사 절단 또는 데이터 손상이 자주 발생한다
...
불안전한 타입이며, 가상 함수 테이블이 손상(overwrite)된다.
...
이 또한 불안전한 타입이며, 가상 함수 테이블이 손상(overwrite)된다.

##### Enforcement

- 'memset' 또는 'memcpy'에 복사 미정 타입(non-trivially copyable type)을 전달할 경우 지적하라
...
* 아무것도 하지 않고 예외를 던지는 멤버가 많은 클래스를 찾아낸다
...
예를 들어 ABI(Application Binary Interface)를 사용하는 지점에서 이런 작업이 필요하다.
...
`union`은 어떤 멤버가 저장되었는지 추적하지 않기 때문에 프로그래머가 정확하게 사용해야 한다; 이는 필연적으로 오류를 발생시키지만, 이를 보완할 방법은 있다
