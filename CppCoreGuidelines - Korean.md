...
* [CP.1: 코드가 다중 스레드 프로그램의 일부로 동작할 것이라 가정하라](#Rconc-multi)
...
* [CP.mess: 메시지 전달(Message passing)](#SScp-mess)
* [CP.vec: 벡터화(Vectorization)](#SScp-vec)
* [CP.free: 락-프리 프로그래밍(Lock-free programming)](#SScp-free)
* [CP.etc: 기타 동시성 규칙들(Etc. concurrency rules)](#SScp-etc)

### <a name="Rconc-multi"></a>CP.1: 코드가 다중 스레드 프로그램의 일부로 동작할 것이라 가정하라

##### 이유

지금 당장 동시성 프로그래밍을 하지 않는다고 해서, 앞으로도 계속 그럴거라고 장담할 수는 없다.
코드는 재사용 될 수 있다.
스레드를 사용하는 라이브러리는 프로그램의 다른 부분에서 사용될 수 있다.
이로 인해 라이브러리 코드에는 이 규칙을 가장 시급히 적용해야 하며, 독립 실행 애플리케이션에는 시급하게 적용하지 않아도 된다.
그러나, 잘라내기 및 붙여넣기의 마법 덕분에 예기치 않은 곳에서 코드 조각이 나타날 수 있다.
...
`cached_computation` 함수는 단일 스레드 환경에서는 완벽하게 동작하지만, 다중 스레드 환경에서는 두 개의 `static` 변수가 데이터 경쟁에 빠져 정의되지 않은 동작을 한다.

위 예시를 다중 스레드 환경에서도 안전하게 동작시키는 몇 가지 방법이 있다.

* 동시성 관련 문제를 호출자에게 위임하라.
* `static` 변수를 `thread_local`으로 표시하라. (하지만 캐싱 효과는 줄어들 수 있다)
* 동시성 제어를 구현하라. 예를 들어, 두 static 변수를 static 락으로 보호하라. (하지만 성능이 저하될 수 있다)
* 호출자에게 캐시에 사용할 메모리를 제공해 메모리 할당과 동시성 관련 문제를 위임하라.
* 다중 스레드 환경에서 빌드되거나 실행되지 않도록 하라.
* 단일 스레드 환경과 다중 스레드 환경을 위한 구현을 각각 제공하라.

##### 예외

다중 스레드 환경에서 절대로 실행되지 않는 코드.
...
데이터 경쟁을 피하지 못하면, 제대로 동작한다고 보장할 수 없고 분석하기 어려운 미묘한 오류들이 계속 발생한다.

##### 참고할 만한 내용

간단한 예로, 두 스레드가 동일한 객체를 동기화하지 않는 채 동시에 접근할 수 있고, 최소 한 스레드에서 쓰기(`const`가 아닌 작업을 수행하는)를 한다면, 데이터 경쟁 상태가 된다.
동기화를 사용해 데이터 경쟁을 제거하는 방법에 대해 더 자세한 정보는 동시성을 다루는 좋은 책을 참고하길 바란다.
...
id를 증가시키는 부분에서 데이터 경쟁이 발생하는데, 아래와 같은 문제를 포함해 여러 가지 문제가 발생한다.

* 스레드 A는 `id`의 값을 로드하고, 운영체제 문맥은 다른 스레드가 수백 개의 ID를 생성하는 일정 기간 A를 전환한다. 스레드 A는 다시 실행되고, `id`는 스레드 A가 읽은 `id`에 1을 더한 값으로 다시 기록된다.
* 스레드 A와 B는 `id`를 로드하고 동시에 증가시킨다. 스레드 A와 B는 같은 ID를 얻는다.

일반적으로 지역 static 변수는 데이터 경쟁을 발생시키는 원인이 된다.
...
여기서 `buf`의 원소에 대해 (위험한) 데이터 경쟁이 있다 (`sort`는 읽기와 쓰기를 모두 수행할 것이다).
모든 데이터 경쟁은 위험하다.
여기서, 우리는 스택 상의 데이터에서 발생하는 데이터 경쟁을 가까스로 처리했다.
모든 데이터 경쟁이 이 예처럼 쉽게 발견되는 것은 아니다.
...
이제는, `val`이 변경될 수 있다는 것을 모르는 컴파일러가 5개의 항목이 있는 점프 테이블을 사용하여 `switch`를 구현할 가능성이 높다.
그러면, `[0..4]` 범위를 벗어난 `val`은 프로그램의 어느 곳으로든 이동하게 되고 거기에서 실행이 진행된다.
실제로, 데이터 경쟁이 발생하면 "상황은 예측할 수 없다".
사실, 상황은 더 나 빠질 수 있다: 생성된 코드를 보면 주어진 값에 대해 표류 점프가 어디로 갈지 결정할 수 있다;
이는 보안상 위험할 수 있다.

##### 적용

무엇이든 좋으니 일단 하라.
이 문제를 해결하기 위한 상업 및 오픈소스 도구가 있다. 하지만 해결책은 비용과 알아채지 못한 문제점을 수반한다.
정적 도구는 거짓 양성(False Positive) 반응을 보이는 경우가 있고, 런타임 도구는 비용이 비싸다는 문제가 있다.
...
* 전역 데이터를 사용하지 마라
* `static` 변수를 사용하지 마라
* 스택 상에서 값 타입을 더 많이 사용하라 (그리고 포인터를 너무 많이 전달하지 말아라)
* 불변 데이터(리터럴, `constexpr`, `const`)를 많이 사용하라
