...
우리는 컴퓨터가 동시에 많은 작업을 하기를 원하고, 그렇게 동작하는것 처럼 보이게 만들기도 한다.
단일 프로세서만 사용하여 많은 이벤트를 기다리거나, 많은 데이터 스트림을 동시에 처리하거나 또는 많은 하드웨어 기능을 활용하려는 경우 등 이유는 다양하다.
이러한 문제를 해결하기 위해서 C++은 동시성과 병렬성을 위한 다양한 기능을 제공한다.
이 절에서는 ISO 표준 C++로 동시성과 병렬성을 구현하는데 필요한 일반적인 원칙과 규칙을 설명한다.

스레드는 동시적이거나 병렬적인 프로그래밍의 핵심이다. 스레드 덕분에 동일한 메모리를 공유하면서 프로그램의 여러 인스턴스를 독립적으로 실행할 수 있다.
동시적인 프로그래밍은 여러 가지 이유로 까다롭다. 대표적으로 한 스레드에서 데이터를 읽는 동안 다른 스레드에서 같은 데이터를 덮어쓸 때 발생하는 비정의 동작(undefined behavior)이 있다. 기존의 단일 스레드 코드를 동시적인 코드로 변경하는 것은 `std::async` 나`std::thread`를 추가하는 것만으로 쉽게 끝날 수도 있지만, 동시성이 고려되지 않은 코드라면 전체 코드를 다시 작성해야 할 수도 있다.

이 문서의 동시성/병렬성 규칙은 3가지 목표를 가지고 설계되었다.

* 스레드를 쉽게 제어할 수 있도록 한다.
* C++ 표준 라이브러리로 안전하고 명료하게 스레드를 제어하는 방법을 제시한다.
* 동시성, 병렬성 기법이 기대했던 성능을 보이지 않을 때 무엇을 해야하는지 지침을 제공한다.

C++이 제공하는 동시성 기법은 지금도 계속 발전하고 있다. 특히 중요한 동시성 연산들이 C++11에 많이 소개되었고, C++14에서는 그것들을 개선하여 C++로 동시성 프로그램을 구현하는것이 즐겁고 더 쉬워지기까지 했다.
일부 스레드관련 라이브러리의 사용지침은 시간이 지나면서 가치있는 방향으로 변화가 있을 것이다.

이 절은 많은 시간과 노력이 필요하다.
우선은 동시성에 덜 익숙한 사용자를 위한 규칙으로 시작할 것이니, 동시성 전문가라면 좀 더 기다려주길 바란다.
이 문서에 기여하는 것은 언제나 환영한다. 하지만 동시성에 익숙하지 않은 개발자들이 정확하고, 빠른 동시적인 프로그램을 작성할 수 있도록 배려해주길 바란다.
